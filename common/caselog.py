...
def info(text):
    ...
    with open(file=Dri + '\\logs\\' + f'{str_time}_info.log', mode='a', encoding='utf-8') as f:
        f.write(content + '\n')
...
def error(text):
    ...
    str_time = datetime.now().strftime("%Y%m%d")
    with open(file=Dri + '\\logs\\' + f'{str_time}_error.log', mode='a', encoding='utf-8') as f:
        f.write(content + '\n')
    with open(file=Dri + '\\logs\\' + f'{str_time}_info.log', mode='a', encoding='utf-8') as f:
        f.write(content + '\n')
...
if __name__ == '__main__':
    info('url = ashduioashduiqhwuidhqwudioh')
    error('遇到问题了')
